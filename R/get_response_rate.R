...
get_response_rate <- function(report_text, row_name = "Responded"){
  report_text[stringr::str_detect(report_text, row_name)] %>% # Find the line in the text
    stringr::str_replace_all(row_name, "") %>% # Get rid of the word "Responded"
    stringr::str_replace_all(" ", "") %>% # Get rid of the white space
    as.numeric() %>% # Convert to a number
    dplyr::first() # if there are multiple lines that match, take the first one (if none, return NA)
}
